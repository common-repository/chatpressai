(function($) {
	...
	$(document).on('click', '.chatpressai-actions-inner > div', function(event) {
		...
		if (attr_class == 'copy-to-title') {
			$('#titlediv input[name="post_title"]').val(cont)
			$('#titlediv #title-prompt-text').addClass('screen-reader-text')
		}
		if (attr_class == 'copy-to-edit-screen') {
			tinymce.activeEditor.setContent(cont);
		}
		if (attr_class == 'copy-to-excerpt') {
			$('textarea[name="excerpt"]').val(cont)
		}
		...
		if (attr_class == 'append-to-edit-screen') {
			tinymce.activeEditor.setContent(tinyMCE.activeEditor.getContent() + ' ' + cont);
		}
		$('.chatpressai-actions-inner').hide()
	});

	function get_response(parent) {
		var action = ['Copy to Title', 'Copy to Edit Screen', 'Append to Edit Screen', 'Copy to Excerpt', 'Copy to Clipboard', ];
		if( pagenow == 'chatpressai_page_chat' ) {
